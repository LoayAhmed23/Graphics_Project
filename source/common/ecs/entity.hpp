    ...
    public:
        ...
        T* getComponent(){
            ...
            for (Component* component : components) {
                // Try to dynamically cast the component to T*
                T* castedComponent = dynamic_cast<T*>(component);
                // If the cast is successful (not null), return it
                if (castedComponent) {
        ...
        void deleteComponent(T const* component){
            ...
            for (auto it = components.begin(); it != components.end(); ++it) {
                // Check if the pointer in the list matches the given component pointer
                if (*it == component) {
                    // Delete the component
                    delete *it;
                    // Remove it from the list
